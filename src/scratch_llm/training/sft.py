...
def build_sft_conversation_sources(
    ...
    *,
    config: SFTConfig,
) -> tuple[FiniteConversationSource, ...]:
    ...
    sources: list[FiniteConversationSource] = []
    for index, source_config in enumerate(normalized):
        ...
        if source_config.kind == "jsonl":
            source: FiniteConversationSource = load_jsonl_conversation_source(
                source_config.path,
...
def run_sft_training(
    ...
    *,
    ...
    train_loader = SFTConversationLoader(
        tuple(
            WeightedConversationSource(source, source_config.repeat_weight)
